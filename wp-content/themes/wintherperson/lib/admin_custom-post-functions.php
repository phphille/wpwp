...
echo '<input type="hidden" name="custom_meta_box_nonce" value="'.wp_create_nonce(basename(__FILE__)).'" />.
		<input type="hidden" name="wp_custom_attachment_nonce" value="'.wp_create_nonce(basename(__FILE__)).'" />.';
      ...
      if($field['name'] == 'heading'){
        ...
        echo '<tr>
                ...
                switch($field['type']) {
                    ...
                    break;
										case 'image':
												$img = get_post_meta($post->ID, $field['name'], true);
												echo '
												<img src="'.$img.'" alt="'.$img.'"><br/>
												<input type="file" name="'.$field['name'].'" value ="" id="'.$field['id'].'" />
												<br /><span class="description">'.$field['desc'].'</span>';
										break;
                } //end switch
        echo '</td></tr>';
...
function save_attachment() {
    ...
    if(!empty($_FILES['wp_custom_attachment']['name'])) {
        ...
        $supported_types = array('application/jpg','application/png');
        ...
        if(in_array($uploaded_type, $supported_types)) {
            ...
            wp_die("Endast filtyperna jpg eller png är tillåtna.");
        } // end if/else
...
function save_custom_meta($post_id) {
  ...
  if(isset($_POST['custom_meta_box_nonce'])){
    $correct_meta_fields = $custom_meta_fields[$current_screen->id];
		// verify nonce
    if (!wp_verify_nonce($_POST['custom_meta_box_nonce'], basename(__FILE__)) && !wp_verify_nonce($_POST['wp_custom_attachment_nonce'], plugin_basename(__FILE__)))
        return $post_id;
    // check autosave
